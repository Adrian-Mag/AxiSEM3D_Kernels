...
def sph2cart_mpmath(points: np.ndarray, precision=64) -> np.ndarray:
    ...
    Args:
        points (np.ndarray): An array of shape (N, 3) containing the spherical coordinates [rad, lat, lon].
        precision (int): Number of decimal places for arbitrary precision (default is 64).
    Returns:
        np.ndarray: An array containing the Cartesian coordinates [x, y, z] for each input point.
    """
...
def cart2sph_mpmath(points: np.ndarray, precision=64) -> np.ndarray:
    ...
    inclination = np.array([mpmath.asin(val / radius[i]) for i, val in enumerate(z)])
    
    # Calculate azimuthal angle (phi) using mpmath
    azimuth = np.array([mpmath.atan2(y[i], x[i]) for i in range(len(x))])
...
def cart2polar_mpmath(s: np.ndarray, z: np.ndarray, precision=64) -> np.ndarray:
    """
    Transform in-plane cylindrical coordinates (cartesian) to polar coordinates.
    ...
    theta = np.where(s == 0,
                     np.where(z > 0, np.pi / 2, np.where(z < 0, -np.pi / 2, 0)),
                     np.array([mpmath.atan2(z[i], s) for i, s in enumerate(s)]))
    r = np.sqrt(s**2 + z**2)
